...
<div class="card">
	<div class="row">
		<div class="col-md-12">
			<a class="btn btn-primary waves-effect" href="<?php echo base_url('admin/videos'); ?>"> <span class="btn-label mr-1"> <i class="fa fa-arrow-left"></i></span><?php echo trans('back_to_list'); ?></a>
			<a class="btn btn-primary waves-effect" href="<?php echo base_url('watch/') . $slug; ?>" target="_blank"> <span class="btn-label mr-1"> <i class="fa fa-eye"></i></span><?php echo trans('preview'); ?></a>
		</div>
	</div>
	<div class="row">
		<div class="col-md-12">
				...
				<div class="panel-body">
					...
					<div class="form-group">
						...
						<select class="form-control" name="source" id="selected-source">
							<option value="upload" <?php if ($video_source == 'upload') : echo 'selected';
													endif; ?>><?php echo trans('upload'); ?></option>
							<option value="youtube" <?php if ($video_source == 'youtube') : echo 'selected';
							...
							<option value="mp4" <?php if ($video_source == 'mp4') : echo 'selected';
												endif; ?>><?php echo trans('mp4_from_url'); ?></option>
							<option value="mkv" <?php if ($video_source == 'mkv') : echo 'selected';
												endif; ?>><?php echo trans('mkv_from_url'); ?></option>
							<option value="webm" <?php if ($video_source == 'webm') : echo 'selected';
													endif; ?>><?php echo trans('webm_from_url'); ?></option>
							<option value="m3u8" <?php if ($video_source == 'm3u8') : echo 'selected';
													endif; ?>><?php echo trans('m3u8_from_url'); ?></option>
							<option value="embed" <?php if ($video_source == 'embed') : echo 'selected';
					...
					<div class="form-group" <?php if ($video_source == 'upload') : echo 'style="display:none;"';
						...
						<input type="text" name="url" id="url-input-field" value="" class="form-control" placeholder="http://server-2.com/movies/titalic.mp4" <?php if ($video_source != 'upload') : echo 'required';
																																								endif; ?>><br>
					</div>
					<div class="form-group" <?php if ($video_source != 'upload') : echo 'style="display:none;"';
					...
					<div class="form-group">
						<button class="btn btn-primary waves-effect" type="submit"> <span class="btn-label mr-1"> <i class="fa fa-plus"></i></span><?php echo trans('add') ?> </button>
					</div>
					</form>
				</div>
			</div>
			<div class="panel panel-border panel-primary">
				...
				<div class="panel-body">
					...
					<table class="table table-bordered" id="video-list">
						...
						?>
							<tr id="row_<?php echo $video_file['video_file_id']; ?>">
								<td><?php echo $sl; ?></td>
								<td><a href="<?php echo base_url('watch/') . $this->common_model->get_slug_by_videos_id($video_file['videos_id']) . '.html?key=' . $video_file['stream_key']; ?>" target="_blank"><?php echo $video_file['file_source']; ?></a></td>
								<td>
					...
					<div class="pull-right mb-2"><button type="submit" class="btn btn-primary"><?php echo trans('save_order'); ?></button></div>
					<?php echo form_close(); ?>
				</div>
			</div>
		</div>
			...
			<div class="panel panel-border panel-primary">
				...
				<div class="panel-body">
					<div id="download_link_section">
						...
						<button type="submit" class="btn btn-primary waves-effect" id="add-download-link"> <span class="btn-label mr-1"> <i class="fa fa-plus"></i></span>Submit</button><br><br>
						</form>
					</div>
				</div>
			</div>
