...
class Genre_model extends CI_Model
{
    ...
    public function get_genre_name_by_slug($slug)
    {
        $result = count($this->db->get_where('genre', array('slug' => $slug))->result_array());
        if ($result > 0) {
            return $this->db->get_where('genre', array('slug' => $slug))->row()->name;
        } else {
            return "Unknown";
        }
    }
    ...
    public function get_genre_description_by_id($genre_id)
    {
        $result = count($this->db->get_where('genre', array('genre_id' => $genre_id))->result_array());
        if ($result > 0) {
            return $this->db->get_where('genre', array('genre_id' => $genre_id))->row()->description;
        } else {
            return "Unknown";
        }
    }
    ...
    public function fetch_genre_video_by_slug( $start = 0,$limit = 16, $slug)
    {
        $genre_id   = $this->db->get_where('genre', array('slug' => $slug))->row()->genre_id;
