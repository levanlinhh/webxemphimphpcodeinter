...
class Country_model extends CI_Model
{
    ...
    public function get_country_name_by_slug($slug)
    {
        $result = count($this->db->get_where('country', array('slug' => $slug))->result_array());
        if ($result > 0) {
            return $this->db->get_where('country', array('slug' => $slug))->row()->name;
        } else {
            return "Unknown";
        }
    }
    ...
    public function get_country_description_by_id($country_id)
    {
        $result = count($this->db->get_where('country', array('country_id' => $country_id))->result_array());
        if ($result > 0) {
            return $this->db->get_where('country', array('country_id' => $country_id))->row()->description;
        } else {
            return "Unknown";
        }
    }
