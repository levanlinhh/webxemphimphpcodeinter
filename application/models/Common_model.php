...
class Common_model extends CI_Model
{
    ...
    function get_img($type = '', $id = '')
    {
        ...
        elseif (film_config('active_theme') == 'flix') :
    ...
    public function generator_sitemap()
    {
        ...
        $landing_page_enable = film_config('landing_page_enable');

        $xml = new SimpleXMLElement("<?xml version='1.0' encoding='UTF-8' ?><?xml-stylesheet type='text/xsl' href='/stylesheet.xsl'?>\n" . '<urlset xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.sitemaps.org/schemas/sitemap/0.9 http://www.sitemaps.org/schemas/sitemap/0.9/sitemap.xsd" xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" />');
        // home page page
        ...
        if ($landing_page_enable == '1') :
            $url = $xml->addChild('url');
            $url->addChild('loc', base_url('all-movies'));
            $url->addChild('priority', '1.0');
            $url->addChild('changefreq', 'hourly');
            $url->addChild('lastmod', date('c', time()));
        endif;
        ...
        // movie page
        $url = $xml->addChild('url');
        $url->addChild('loc', base_url('list/phim-le'));
        ...
        // tv series page
        $url = $xml->addChild('url');
        $url->addChild('loc', base_url('list/phim-bo'));
        $url->addChild('priority', '0.9');
        $url->addChild('changefreq', 'daily');
        $url->addChild('lastmod', date('c', time()));
        ...
        if ($live_tv_publish == '1') :
            $url = $xml->addChild('url');
            $url->addChild('loc', base_url('live-tv'));
            $url->addChild('priority', '0.9');
            $url->addChild('changefreq', 'daily');
            $url->addChild('lastmod', date('c', time()));
        endif;
        ...
        foreach ($genres as $genre) :
            $url = $xml->addChild('url');
            $url->addChild('loc', base_url("genre/" . $genre['slug']));
            $url->addChild('priority', '0.9');
            $url->addChild('changefreq', 'daily');
            $url->addChild('lastmod', date('c', time()));
        endforeach;
        ...
        foreach ($countries as $country) :
            $url = $xml->addChild('url');
            $url->addChild('loc', base_url("country/" . $country['slug']));
            $url->addChild('priority', '0.9');
            $url->addChild('changefreq', 'daily');
            $url->addChild('lastmod', date('c', time()));
        endforeach;
        ...
        for ($i = $current_year; $i > $end_year; $i--) :
            $url = $xml->addChild('url');
            $url->addChild('loc', base_url("year/" . $i));
            $url->addChild('priority', '0.9');
            $url->addChild('changefreq', 'daily');
            $url->addChild('lastmod', date('c', time()));
        endfor;
        ...
        foreach ($pages as $page) :
            $url = $xml->addChild('url');
            $url->addChild('loc', base_url("page/" . $page['slug']));
            $url->addChild('priority', '0.9');
            $url->addChild('changefreq', 'daily');
            $url->addChild('lastmod', date('c', time()));
        endforeach;
        ...
        if ($blog_enable == '1') :
            $url = $xml->addChild('url');
            $url->addChild('loc', base_url('blog'));
            $url->addChild('priority', '0.9');
            $url->addChild('changefreq', 'daily');
            $url->addChild('lastmod', date('c', time()));
        ...
        foreach ($movies as $movie) :
            $url = $xml->addChild('url');
            $url->addChild('loc', base_url("watch/" . $movie['slug']));
    ...
    {
        // season order
        $season_order   =   film_config('video_file_order');
    ...
    {
        // season order
        $season_order   =   film_config('season_order');
        if ($season_order == 'DESC') :
            $season_order = 'ASC';
        else :
            $season_order = 'DESC';
    ...
    {
        // episode order
        $episode_order   =   film_config('episode_order');
