...
class User extends Home_Core_Controller
{
        ...
        if ($param1 == 'do_signup') {
            ...
            if ($registration_enable == "1") :
                ...
                if ($this->form_validation->run() == FALSE) :
                    ...
                    if ($email_exist) :
                        $this->session->set_flashdata('sign_up_error', 'Đăng ký không thành công. Email đã tồn tại trên hệ thống');
                    else :
                        $data['join_date']        = date('Y-m-d H:i');
                        $data['last_login']       = date('Y-m-d H:i');
                        ...
                        try {
                            ...
                            $email_error = 'Không thể gửi email, Vui lòng kiểm tra cài đặt thư của bạn.';
                        }
                        ...
                        $this->session->set_flashdata('sign_up_success', 'Đăng ký thành công. Bây giờ bạn có thể đăng nhập vào hệ thống.');
                        redirect(base_url() . 'user/registration', 'refresh');
                    endif;
                endif;
            else :
                $this->session->set_flashdata('sign_up_error', 'Đăng ký bị vô hiệu hóa bởi quản trị viên.');
                redirect(base_url() . 'user/registration', 'refresh');
            endif;
        }
        redirect(base_url() . 'user/registration', 'refresh');
        ...
        if ($param1 == 'do_reset') {
            ...
            if ($user_exist) {
                ...
                try {
                    ...
                    $this->session->set_flashdata('reset_error', 'Không thể gửi email, Vui lòng kiểm tra cài đặt thư của bạn.');
                    redirect(base_url() . 'user/forget_password', 'refresh');
                }



                $this->session->set_flashdata('reset_success', 'Vui lòng kiểm tra email của bạn để hoàn thành việc đặt lại mật khẩu.');
                redirect(base_url() . 'user/forget_password', 'refresh');
            } else {
                $this->session->set_flashdata('reset_error', 'Không tìm thấy email trên hệ thống');
                redirect(base_url() . 'user/forget_password', 'refresh');
            }
        }
    ...
    function complete_reset($param1 = '', $param2 = '')
    {
        if ($param1 == 'save') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->session->set_flashdata('login_success', 'Mật khẩu đã được thay đổi');
                redirect(base_url() . 'user/login', 'refresh');
            }
        }
        ...
        if (isset($token) && $token != '') {
            ...
            if ($token_exist) {
                ...
                $this->session->set_flashdata('reset_error', 'Mã không hợp lệ..');
                redirect(base_url() . 'user/forget_password', 'refresh');
            }
        } else {
            $this->session->set_flashdata('reset_error', 'Mã không hợp lệ..');
    ...
    function manage_profile()
    {
        ...
        $data['page_title']     = 'Cập nhật thông tin hồ sơ';
    ...
    function profile($param1 = '', $param2 = '')
    {
        ...
        if ($param1 == 'update') {
            ...
            if ($this->form_validation->run() == FALSE) :
                ...
                $this->session->set_flashdata('success', 'Thông tin hồ sơ đã được cập nhật.');
                redirect(base_url() . 'user/update_profile/', 'refresh');
            endif;
        }
    ...
    function favorite($param1 = '', $param2 = '')
    {
        ...
        $data['title']          = 'Phim yêu thích của tôi';
    ...
    function watch_later($param1 = '', $param2 = '')
    {
        ...
        $data['title']          = 'Phim xem sau của tôi';
        $this->db->order_by('wish_list_id', 'desc');
    ...
    function change_password($param1 = '', $param2 = '')
    {
        ...
        if ($param1 == 'update') {
            ...
            if ($current_password == $password && $new_password == $retype_new_password) {
                ...
                $this->session->set_flashdata('error', 'Mật khẩu không khớp.');
            }
    ...
    function do_login()
    {
        ...
        if ($this->form_validation->run() == FALSE) :
            ...
            if ($login_status == 'success') :
                ...
                $this->session->set_flashdata('login_error', 'Tên người dùng và mật khẩu không khớp ..');
                redirect(base_url() . 'user/login', 'refresh');
            endif;
        endif;
    }
    ...
    function report_movie($param1 = '')
    {
        ...
        if ($this->input->post('audio') != "" && $this->input->post('audio') != NULL) {
            $data['issue']      .= $i . '- Âm thành ' . $this->input->post('audio') . '<br>';
            $i++;
        }
        if ($this->input->post('subtitle') != "" && $this->input->post('subtitle') != NULL) {
            $data['issue']   .= $i . '- Phụ đề ' . $this->input->post('subtitle') . '<br>';
            $i++;
        }
        ...
        if ($this->form_validation->run() == FALSE) :
            ...
            $this->session->set_flashdata('success', 'Đã gửi báo cáo thành công.');
            redirect($this->agent->referrer(), 'refresh');
        endif;
    }
}
