...
class Home extends Home_Core_Controller
{
	...
	public function index()
	{
		...
		if (film_config('active_theme') == 'flix') :
	...
	{
		$data['title'] = 'Khiếu nại bản quyền';
		$data['page_name'] = 'dmca';
		$this->load->view('theme/' . $this->active_theme . '/index', $data);
	}
	...
	{
		$data['title'] = 'Chính sách riêng tư';
	...
	function contact_process()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($this->email_model->contact_email($name, $email, $message)) {
				$this->session->set_flashdata('success', 'Tin nhắn gửi thành công.');
			} else {
				$this->session->set_flashdata('error', 'Oops! Đã xảy ra lỗi.');
				redirect(base_url() . 'contact-us', 'refresh');
			}
		}
		redirect(base_url() . 'contact-us', 'refresh');
	...
	function send_movie_request()
	{
		...
		if ($this->form_validation->run() == FALSE) :
			...
			$this->session->set_flashdata('success', 'Đã gửi yêu cầu thành công.');
			redirect($this->agent->referrer());
		endif;
	}
